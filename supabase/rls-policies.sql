-- ===============================================
-- EDUQUINHA - ROW LEVEL SECURITY POLICIES
...
CREATE POLICY "Admins can manage all attendance" ON presenca
  FOR ALL USING (is_admin());

-- Professors can manage attendance for their students
CREATE POLICY "Professors can manage their students' attendance" ON presenca
  FOR ALL USING (is_professor() AND teaches_student(aluno_id));

-- Parents can view their children's attendance
CREATE POLICY "Parents can view their children's attendance" ON presenca
  FOR SELECT USING (is_responsavel() AND is_responsible_for_student(aluno_id));
...
-- ===============================================
-- 8. FINANCEIRO POLICIES
-- ===============================================

-- Admins can manage all financial records
CREATE POLICY "Admins can manage all financeiro" ON financeiro
  FOR ALL USING (is_admin());

-- Parents can view their own financial records
CREATE POLICY "Parents can view their financeiro" ON financeiro
  FOR SELECT USING (
    is_responsavel() AND (
      responsavel_id = auth.uid() OR
...
CREATE POLICY "Parents can view their children's photos" ON fotos
  FOR SELECT USING (
    is_responsavel() AND 
    is_responsible_for_student(aluno_id) AND
    (privada = false OR professor_id IN (
      SELECT t.professor_id FROM turmas t
      JOIN alunos a ON a.turma_id = t.id
      WHERE a.id = aluno_id
    ))
...
CREATE POLICY "Admins can manage all eventos" ON eventos
  FOR ALL USING (is_admin());

-- Professors can create and view events for their classes
CREATE POLICY "Professors can manage their class events" ON eventos
  FOR ALL USING (
    is_professor() AND (
      turma_id IN (SELECT id FROM turmas WHERE professor_id = auth.uid()) OR
      turma_id IS NULL
    )
  );

-- Parents can view events for their children's classes
CREATE POLICY "Parents can view relevant events" ON eventos
  FOR SELECT USING (
    is_responsavel() AND (
      turma_id IS NULL OR
      turma_id IN (
