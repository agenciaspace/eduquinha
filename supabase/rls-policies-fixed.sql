-- ===============================================
-- EDUQUINHA - RLS POLICIES FIXED FOR YOUR STRUCTURE
...
-- Professors can view and update students in their classes
CREATE POLICY "Professors can manage their students" ON alunos
  FOR ALL USING (is_professor() AND teaches_student(id));
...
-- ===============================================
-- 7. COMUNICADOS POLICIES (SIMPLIFIED - NO DESTINATARIOS/TURMA_ID)
-- ===============================================

-- Admins can manage all comunicados
CREATE POLICY "Admins can manage all comunicados" ON comunicados
  FOR ALL USING (is_admin());

-- Professors can create comunicados
CREATE POLICY "Professors can create comunicados" ON comunicados
  FOR INSERT WITH CHECK (is_professor());

-- Everyone can view all comunicados (simplified for now)
CREATE POLICY "Everyone can view comunicados" ON comunicados
...
CREATE POLICY "Users can update their messages" ON mensagens
  FOR UPDATE USING (
    remetente_id = auth.uid() OR 
    destinatario_id = auth.uid()
  );
...
-- Professors can manage activities
CREATE POLICY "Professors can manage atividades" ON atividades
  FOR ALL USING (is_professor());

-- Parents can view activities of their children
CREATE POLICY "Parents can view atividades" ON atividades
  FOR SELECT USING (
    is_responsavel() AND (
      aluno_id IS NULL OR 
      is_responsible_for_student(aluno_id)
